 ...
 * returns the most recent completed triage. Needs a prior successful extraction to supply the rootfs — checked by
 * `providers/rootfs-gate.ts`, which distinguishes an extraction nobody ran from one that ran and found no rootfs.
 */
...
const STAGE: RootfsStage = { stage: 'decompile', needs: 'binary triage' };
