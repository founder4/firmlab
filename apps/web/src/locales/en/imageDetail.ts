...
export const imageDetail = {
  ...
  binaries: {
    listTitle: (n: number) => `Binaries (${n})`,
    listSub: 'Every ELF from the extracted rootfs, with architecture from its header. Select one to triage it.',
    colPath: 'Path',
    colArch: 'Arch',
    colHardening: 'Hardening',
    colImports: 'Notable imports',
    colNet: 'Net',
    triageTitle: 'Binary triage (radare2)',
    triageSub: 'Static triage of a binary from the extracted rootfs: headers, imports, symbols, strings.',
    noRootfs: (section: string) => `No extracted rootfs yet — run extraction on the ${section} tab first.`,
    pathPlaceholder: 'rootfs-relative path, e.g. bin/busybox',
    triaging: 'Triaging…',
    triage: 'Triage binary',
    unavailable: 'Triage unavailable — check the path, or install radare2.',
    nx: (on: boolean) => `NX ${on ? 'on' : 'off'}`,
    canary: (on: boolean) => `canary ${on ? 'on' : 'off'}`,
    pic: (on: boolean) => `PIC ${on ? 'yes' : 'no'}`,
    funcs: (n: number) => `${n} funcs`,
    importsTitle: (n: number) => `Imports (${n})`,
    colSymbol: 'Symbol',
    colLibrary: 'Library',
    symbolsTitle: (n: number) => `Symbols (${n})`,
    colSymbolName: 'Name',
    colSymbolType: 'Type',
    stringsTitle: (n: number) => `Strings (${n})`,
    colAddress: 'Address',
    colValue: 'Value',
  },

  ghidra: {
    title: 'Decompilation (Ghidra)',
    sub: 'Full pseudocode via Ghidra headless — needs the optional Ghidra image layer.',
    decompiling: 'Decompiling…',
    decompile: 'Decompile with Ghidra',
    unavailable: 'Ghidra not installed — build the image with the optional Ghidra layer.',
    decompiled: (n: number, binary: string) => `${n} functions decompiled from ${binary}.`,
  },
