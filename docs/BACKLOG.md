...
- ✅ **The guest repair is wired, and it MADE THE WR940N ANSWER** (2026-07-30, `6ad7423` + `c0eb9b2`). `guest-repair.ts` had been pure, unit-tested and had ZERO callers since its commit: it composed the line and nothing wrote it. **Measured, with a control, on the deployed build:** the same WR940N image, both passes freshly rebuilt so the rebuild is not the variable — **with the repair `open: [{host:35437,guest:80},{host:35438,guest:443}]`, without it `open: []`.** First reachable service the full-system rung has produced on this corpus, and it unblocks `webprobe`, service enumeration and any protocol-aware test, all of which needed a live daemon.
  **The mechanism is NOT confirmed, and the result says so.** `ruleset.ran` came back FALSE — the appended line never printed its markers — so the image demonstrably carried the repair and the repair's own self-report is missing. The effect is measured; the causal path through `iptables-stop` is not. That is exactly the outcome the module's header demanded be reportable (*"a repair that cannot tell you it was unnecessary is not a diagnosis"*), and `interventions` carries both facts onto every finding from that boot.
  Design premises verified on real bytes BEFORE any code: all three routers ship `etc/rc.d/iptables-stop` at **284 identical bytes**, none calls it from `rcS`, all three busybox carry a `ping` applet, and the WR940N and MR3220 carry **no `sleep` at all** — which is why the repair's timer is a ping. `rcS` restored byte-for-byte (same sha256, 795 bytes) after both boots.
  The missing pure piece was not the line but the DISPOSITION: `interventions: []` already meant "the image as shipped", and that is only true if the firmware was examined — with the flag off nobody looked, and the same empty list would be a claim about a look that never happened. `describeRepairDisposition` adds `attempted`, and the four outcomes (flag off / no rootfs / examined and untouched / modified) produce four different sentences. `FIRMLAB_EMU_REPAIR` joins the flag catalogue with prose in both languages, off by default.
- ✅ **A cached disk image was reused under the WRONG repair disposition** (2026-07-30, `c0eb9b2`) — found by the first real repaired boot, one commit after the wiring, and the **third instance this session** of *"a guard is only as good as its SUCCESS path"*. Reuse compared mtimes alone, so with the flag ON a real WR940N boot was served an image built WITHOUT a repair and returned `repair: undefined`: the operator asked for an intervention and silently did not get one — worse than an absent field, a result about an artefact nobody described. `imageReusable` now requires the disposition the image was BUILT with to match the one the boot wants, recorded in a sidecar written only after a successful mkfs. **Both directions matter and the second one earned its keep immediately:** the unrepaired CONTROL boot above would otherwise have reused the repaired image and reported open ports for a run with no repair, inverting the experiment.
