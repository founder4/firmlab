...
- ▢ **The deployed image will not have yara until the next `--tools` rebuild.** The recipe change lands in `Dockerfile.tools`, which `deploy.sh` rebuilds only on `--tools`; a plain deploy reuses the cached base. Until that runs, `/api/tools` keeps reporting yara absent and the provider keeps degrading honestly — correct behaviour, and worth stating so the fix is not assumed live.
