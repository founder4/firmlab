...
- ⚠ **The listing cap hides exactly the binary the rank exists to promote.** `selectFindings` orders equal-severity candidates smallest-first, so on a rootfs with more candidates than the 45-item cap the LARGEST is dropped first — and the exposed daemon is always the largest. On the WDR3600 that is a 1.7 MB `usr/bin/httpd`, autostart on port 80, 5 unbounded copies, which the rank never sees because `BinVulnResult` exposes only the post-cap `findings`. Fix shape: have `runBinVuln` also return the exposure-relevant candidates it dropped, or let the rank read `candidates` rather than `findings` — but the leads would then name binaries absent from the ledger, which needs its own answer.
- ▢ **`runnable` lets uClibc's shared objects through, and they are what smallest-first buys.** `lib/libutil-0.9.30.so` and `lib/libcrypt-0.9.30.so` pass the filter because uClibc gives them an entry point, and they are the two smallest candidates on all three TP-Link images — so they take 1–2 of every 3 probes and return "search space exhausted" in 1–8 steps, 0.9–1.6 s. The cheapest possible non-answer. The `.so` filter's premise is that a library has no entry point to be reachable from; the real predicate is whether that entry point is a *program*.
...
- ⚠ **The emulated guest has outbound internet, and the workbench's headline claim says it should not.** `-netdev user` gives the guest unrestricted egress: during pass two the WDR3600 reached **three public NTP servers**, now visible in the frame capture. This predates the two-pass work and is not caused by it, but it sits directly against *"with every flag off: no network, no cost, deterministic behaviour"* and against `FIRMLAB_RESEARCH` being the only internet-touching analysis lane — a booted firmware can phone home from a deployment that has every lane switched off. `restrict=on` on the netdev keeps `hostfwd` working while cutting egress, and is the obvious fix; the question worth answering first is whether any rung *depends* on outbound (a firmware that hangs without DNS would change behaviour under the fix, which is itself worth knowing).
...
- ⚠ **The rule-matched branch has never met a real yara.** `yara` is absent from the container and the dev host, so the degradation path is the only one validated against reality; the full path was driven against a STUB speaking YARA 4.x's CLI contract, and the output format was written from `cli/yara.c`'s print order rather than captured from a live binary. Per this repo's own lesson, treat the first real-corpus run as a hypothesis and budget for the second and third failure. `Dockerfile.tools` needs `yara` in its first apt block (bookworm ships 4.2.3, which has `--scan-list`, `-e`, `-g`, `-a`); a Debian build lacks the `cuckoo` module, which is exactly what the `missing-module` classification exists to surface.
