...
- ▢ **`ui:up`'s sidecar collides with `deploy.sh` (found 2026-08-04).** `scripts/ui-expose.sh up` creates a
  container named `firmlab-view` outside compose, and the homelab compose now declares a service of the same name,
  so a deploy run while the viewer is up aborts at `docker compose up -d` with a name conflict — after having
  already built and tagged the image. The two are the documented tools for the two halves of one loop (change the
  UI, look at it, ship it), so hitting this is the normal path, not a corner. `deploy.sh` should either drop the
  stray viewer the way it already checks for a port squatter, or `ui-expose.sh` should not claim a name compose
  owns.
