...
| **7 · Dynamic analysis** | The booted service **is** driven now: `webprobe.ts` reproduces command injection (marker/nonce) and path traversal against the live daemon → `confirmed_in_emulation`. **Update-mechanism integrity** is answered statically (`updatepath.ts`, ISTG-FW, see §2). AFL++ fuzzing (file/stdin/network) under OS-primitive isolation. **The emulated guest CAN now be made to answer, and it was measured rather than asserted**: a third pass boots with `init=/bin/sh`, drives the serial console, reads the guest's own `iptables -L INPUT`, runs the firmware's own `/etc/rc.d/iptables-stop` and reads it again — WR940N **DROP → ACCEPT, guest 80 and 443 answering**. The homelab arms this attributable console pass with `FIRMLAB_EMU_CONSOLE`; it runs only after an un-intervened boot answered nothing, and earns its OWN ledger row carrying its interventions. As soon as an HTTP forward answers, `emulate-system.ts` now executes the bounded web probe **before that QEMU pass is torn down** and records pass, port, request budget, findings and interventions. `scripts/verify-full-system-reliability.sh` is the five-boot acceptance test. The earlier `FIRMLAB_EMU_REPAIR` route stays INERT and disabled: appending to the end of `rcS` was measured not to execute on WR940N. Open: auth-bypass / default-creds / POST-body injection and protocol-aware service testing (MQTT/CoAP). U-Boot posture is read from the env, never interacted with on a live console. | ◐ |
