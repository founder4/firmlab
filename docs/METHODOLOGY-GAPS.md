...
| **7 · Dynamic analysis** | The booted service **is** driven now: WebProbe v2 uses a clean control and two independent challenges for command injection, plus a missing-file control for traversal; only the live emulation path may write `confirmed_in_emulation`, while manual probes remain `needs_runtime_reproduction`. It reports planned/attempted/completed points and budget skips instead of claiming all discovered endpoints. **Update-mechanism integrity** is answered statically (`updatepath.ts`, ISTG-FW, see §2). AFL++ fuzzing (file/stdin/network) runs with bounded resources and operator authorisation. **The emulated guest CAN now be made to answer, and it was measured rather than asserted**: a third pass boots with `init=/bin/sh`, drives the serial console, reads the guest's own `iptables -L INPUT`, runs the firmware's own `/etc/rc.d/iptables-stop` and reads it again — WR940N **DROP → ACCEPT, guest 80 and 443 answering**. The homelab arms this attributable console pass with `FIRMLAB_EMU_CONSOLE`; it runs only after an un-intervened boot answered nothing, and earns its OWN ledger row carrying its interventions. As soon as an HTTP **or HTTPS** forward answers, `emulate-system.ts` executes the bounded web probe **before that QEMU pass is torn down** and records protocol, pass, port, request budget, findings and interventions. Self-signed and TLS-1.0-era firmware is handled by a transport that rejects every non-loopback destination before applying its per-socket TLS exception; global certificate verification remains intact. `scripts/verify-full-system-reliability.sh` requires both protocols in its five-boot acceptance test. The earlier `FIRMLAB_EMU_REPAIR` route stays INERT and disabled: appending to the end of `rcS` was measured not to execute on WR940N. Open: auth-bypass / default-creds / POST-body injection and protocol-aware service testing (MQTT/CoAP). U-Boot posture is read from the env, never interacted with on a live console. | ◐ |
...
| **MEM · Memory** (readout protection, at-rest crypto, key extraction) | Embedded key material + effectively-public detection (`keys.ts`), unsalted-hash lookup against public tables (`hashlookup.ts`, never cracking). Encryption-at-rest inferred from entropy, **plus a loader-derived-key lead** (`@firmlab/core` `detectLoaderDerivedKey`, consumed by `uboot.ts` W-bootloader): nearby decrypt/KDF strings plus independent encrypted-partition evidence produce `needs_runtime_reproduction` with the candidate recipe, never a derived key or a claim that its data flow is proven. `encrypted.ts` W8 recognizes a structurally consistent `ENC1` header and names paired-loader inspection, but remains `blocked_by_security` until loader evidence exists. **RDP/readout-protection posture still not assessed** (needs the chip/vector table). | ◐ |
...
**bounded execution with explicit approval** (rlimits and optional network namespace, deliberately reported as
partial rather than a sandbox), a **cited external-intel** track with an
